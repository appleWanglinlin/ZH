...
const user = {
  ...
  actions: {
    ...
    LoginByUsername({ commit }, userInfo) {
      return new Promise((resolve, reject) => {
        const { tenantId, username, password, type, key, code, fp } = userInfo
        const data = { tenantId, username, password: md5(password), type, key, code, fp }
        loginByUsername(data).then(data => {
          if (data.error_description) {
            ...
            // 存储数据的唯一值，用于localStoreage中的userInfo发生变化时校验数据是否合法
            data.hash = null
    ...
    LoginBySocial({ commit }, userInfo) {
      return new Promise((resolve) => {
        loginBySocial(userInfo.tenantId, userInfo.source, userInfo.code, userInfo.state).then(data => {
          if (data.error_description) {
            Message({
              message: data.error_description,
              type: 'error'
    ...
    refreshToken({ state, commit }) {
      return new Promise((resolve, reject) => {
        refreshToken(state.refreshToken, state.tenantId).then(data => {
          commit('SET_TOKEN', data.access_token)
          commit('SET_REFRESH_TOKEN', data.refresh_token)
          resolve()
        }).catch(error => {
    ...
    // 登出
    LogOut({ commit }, needRequest = true) {
      if (!needRequest) {
        resetData(commit)
  ...
  mutations: {
    SET_TOKEN: (state, token) => {
      setToken(token)
      state.token = token
      if (getStore({ name: 'token', type: 'local' }) !== token) {
        setStore({
          name: 'token',
          content: state.token
        })
